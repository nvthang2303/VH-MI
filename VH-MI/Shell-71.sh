...
# Giải mã apk
unapk () {
[ "$(echo "$Lapp" | grep -cm1 '/data/')" == 1 ] && cp -f "$Lapp" $Dtool/apks/$Lpkg.apk || cp -f "$Lapp" $TMPDIR/rac/$Lpkg.apk
[ -e $Dtool/apks/$Lpkg.apk ] && cp -f $Dtool/apks/$Lpkg.apk $TMPDIR/rac/$Lpkg.apk
...
echo "$MODPATH$( pm path $Lpkg | cut -d : -f2 )" > "$TMPDIR/rac/$Lpkg.txt"
}

# Đóng gói lại apk
repapk () {
if [ -e $TMPDIR/rac/$1.txt ];then
Pathapk="$(cat $TMPDIR/rac/$1.txt)";
...
const/4 v0, 0x0'
Tkllg="$(grep -Rl "$tk" $TMPDIR/rac/$Lpkg/classes*)"
...
modset () {
sed -i 's|sget-boolean v1, Lmiui/os/Build;->IS_GLOBAL_BUILD:Z|const/4 v1, 0x1|' $TMPDIR/rac/$Lpkg/classes*/com/android/settings/MiuiSettings.smali
...
cp -f "$Dtool/apks/com.miui.weather2.apk" "$TMPDIR/rac/com.miui.weather2.apk"
fi
VipApk com.miui.weather2 modthoitiet
rm -fr $Dbackup/system/*/overlay/*com.miui.weather2
echo "$MODPATH/system/app/com.miui.weather2/com.miui.weather2.apk" > $TMPDIR/rac/com.miui.weather2.txt
fi
Xan
# Các lựa chọn 
if [ "$getapps" == 2 ];then
